// packages/edit/src/dispatch/group-e-dispatchers.ts
...
import {
  ...
  type GroupDContractEntityResolutionRequestV1,
  type MediaRefV1,
  type OperationPlanningChoiceV1,
  type GroupECreationOperationV1,
  type GroupECreationResultRoleV1,
...
import { declarationContractBindingKeys } from './group-d-dispatchers.js'
import {
  curatedMediaEntityV1,
  exactMediaRefV1,
  mediaContractEntityRefV1,
  resolveMediaReferenceV1,
} from './group-f-dispatchers.js'
import {
  canonicalGroupEParameterRefV1 as canonicalParameterRef,
  canonicalGroupEProcedureRefV1 as canonicalProcedureRef,
  exactGroupEParameterRefV1 as exactParameterRef,
  groupEParameterLineageInV1 as parameterLineageIn,
  groupEProcedureLineageInV1 as procedureLineageIn,
  groupEProcedureRecordAtV1 as procedureRecordAt,
  groupEProcedureSignatureStateSha256V1 as procedureSignatureStateSha256V1,
  planningGroupEParameterV1 as planningParameter,
  planningGroupEProcedureV1 as planningProcedure,
  resolveGroupEParameterSelectionV1 as resolveParameterSelection,
  resolveGroupEProcedureSelectionV1 as resolveProcedureSelection,
} from './group-e-procedure-authority.js'
import type {
...
export interface GroupEProductionDynamicResultSlotV1
{
...
type GroupEResultSlotV1 =
  GroupCProductionResultSlotV1 | GroupEProductionDynamicResultSlotV1

interface GroupEProductionOperationResultV1
{
  ...
  readonly dynamicSlots?: readonly GroupEProductionDynamicResultSlotV1[]
  readonly postconditionSha256: string
}

interface GroupEPlanningFactProjectionV1
{
  readonly kind: 'group-e-procedure-planning-fact-set'
  readonly schemaVersion: 1
  readonly operationKind: GroupEOperationV1['kind']
  readonly opId: string
...
interface ResolvedGroupEDispatchV1
{
  readonly operation: GroupEOperationV1
  readonly canonicalOperation: GroupEOperationV1
...
{
  readonly binding: GroupEFutureContractBindingV1
  readonly slot: GroupEResultSlotV1
...
function resolveGroupEDispatch(
  context: ProductionOperationContextV1,
...
function groupEPlanningFactProjection(
  resolved: ResolvedGroupEDispatchV1
): GroupEPlanningFactProjectionV1
{
  return {
...
function productionGroupEPlanningFactProjectionV1(
  context: ProductionOperationContextV1,
  operation: GroupEOperationV1
): GroupEPlanningFactProjectionV1
{
  return groupEPlanningFactProjection(resolveGroupEDispatch(context, operation))
}

export function productionGroupEPlanningFactSetSha256V1(
  context: ProductionOperationContextV1,
  operation: GroupEOperationV1
): string
{
  return semanticHashV1(
    'resolved-plan',
    productionGroupEPlanningFactProjectionV1(context, operation)
  )
}
...
{
  const planningFactSetSha256 = productionGroupEPlanningFactSetSha256V1(
    context,
    operation
  )
...
export function productionGroupESimplePlanningCompletionV1(
  context: ProductionOperationContextV1,
  goal: Extract<
...
export function productionGroupEUpdateSignatureChoiceSlotsV1(
  context: ProductionOperationContextV1,
  goal: Extract<
    SemanticEditOperationGoalV1,
...
export function productionGroupEUpdateSignaturePlanningCompletionV1(
  context: ProductionOperationContextV1,
  goal: Extract<
    SemanticEditOperationGoalV1,
    { readonly kind: 'procedure.updateSignature' }
...
{
  const expandedSlots = productionGroupEUpdateSignatureChoiceSlotsV1(
    context,
    goal
  )
  if (
...
interface ReconciledGroupELineageV1
{
...
interface CollectedGroupEResultSlotsV1
{
  readonly fixedSlots: readonly GroupCProductionResultSlotV1[]
  readonly dynamicSlots: readonly GroupEProductionDynamicResultSlotV1[]
}
...
function collectGroupEResultSlots(
  context: ProductionOperationContextV1,
  resolved: ResolvedGroupEDispatchV1,
  lineage: ReconciledGroupELineageV1,
  applied: AppliedProcedureOperationV1
): CollectedGroupEResultSlotsV1
{
  const fixedSlots: GroupCProductionResultSlotV1[] = []
  const dynamicSlots: GroupEProductionDynamicResultSlotV1[] = []
...
function groupECreationRoleForSlot(
  slot: GroupEResultSlotV1
): GroupECreationResultRoleV1
{
...
function futureBindingScopeMatches(
  binding: GroupEFutureContractBindingV1,
...
function matchResultBindings(
  context: ProductionOperationContextV1,
  operation: GroupEOperationV1,
  slots: readonly GroupEResultSlotV1[],
  targetBindingKeysForCreation: readonly string[],
  procedureBindingKeysForSlot: (slot: GroupEResultSlotV1) => readonly string[],
  scriptBindingKeysForSlot: (slot: GroupEResultSlotV1) => readonly string[],
...
): readonly ResultBindingMatchV1[]
{
  return Object.freeze(
    slots.map((slot) =>
    {
      const role = groupECreationRoleForSlot(slot)
      const matches = context.contract.entityBindings
        .filter(
          (binding): binding is GroupEFutureContractBindingV1 =>
            binding.bindingKind === 'future' &&
...
function exactContractRef(
  ...
  expectedEntityKind: GroupDContractEntityResolutionRequestV1['expectedEntityKind'],
  expectedEntitySubtype: GroupDContractEntityResolutionRequestV1['expectedEntitySubtype'],
  semanticPath: string
): ContractEntityRefV1
{
...
): string
{
  ...
  return groupECreationContentFingerprintForResultV1({
    project: sourceProject,
    targetIndex: resolved.targetIndex,
    operation: resolved.canonicalOperation as GroupECreationOperationV1,
    descriptor: binding,
    resultRole: groupECreationRoleForSlot(slot),
...
function groupEPropertyAllowed(
...
): ContractScopeV1
{
  ...
  const matches = context.contract.allowedSemanticScopes.filter((scope) =>
  {
    if (
      ...
      !groupEPropertyAllowed(scope, operation)
    )
      return false
    if (scope.locationScope.scopeKind === 'exactEntity')
...
function groupEOperationResult(
  ...
  dynamicSlots: readonly GroupEProductionDynamicResultSlotV1[]
): GroupEProductionOperationResultV1
{
...
function assertGroupEPreconditions(
  context: ProductionOperationContextV1,
  resolved: ResolvedGroupEDispatchV1,
  record: ProcedureRecordV1 | null
): void
...
function groupEOrderedCorrespondence(
  context: ProductionOperationContextV1,
  resolved: ResolvedGroupEDispatchV1,
  lineage: ReconciledGroupELineageV1,
  applied: AppliedProcedureOperationV1
...
function executeResolvedGroupEOperation(
  context: ProductionOperationContextV1,
  resolved: ResolvedGroupEDispatchV1,
  planningFactProjection: GroupEPlanningFactProjectionV1
): ProductionOperationDispatchResultV1
{
  const operation = resolved.operation
  if (
  ...
  assertGroupEPreconditions(context, resolved, record)
  ...
  const lineage = reconcileGroupELineage(
    context,
    resolved,
  ...
  const slots = collectGroupEResultSlots(context, resolved, lineage, applied)
  ...
  const selectedScope = selectGroupEScope(
    context,
    operation,
    scopeEntityKeys,
    targetKeys,
  ...
  const correspondence = groupEOrderedCorrespondence(
    context,
    resolved,
    lineage,
    applied
...
class ProcedureLifecycleProductionOperationDispatcherV1 implements ProductionOperationDispatcherV1
{
  ...
  ): ProductionOperationDispatchResultV1
  {
    ...
    const resolved = resolveGroupEDispatch(
      context,
      operation as ProcedureLifecycleOperationV1
    )
    return executeResolvedGroupEOperation(
      context,
      resolved,
      groupEPlanningFactProjection(resolved)
    )
  }
}
...
export function groupEProductionOperationDispatchersV1(): readonly ProductionOperationDispatcherV1[]
{
  return Object.freeze([
