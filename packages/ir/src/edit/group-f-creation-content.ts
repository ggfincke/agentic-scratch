// packages/ir/src/edit/group-f-creation-content.ts
...
import type { SemanticCreationNormalizationContextV1 } from './group-d-creation-content.js'
import { semanticHashV1 } from './hash-domains.js'

import type { Without } from './internal-types.js'

type GroupFFutureContractBindingV1 = Extract<
  ContractEntityBindingV1,
  {
    bindingKind: 'future'
...
export type GroupFCreationBindingDescriptorV1 = Without<
  GroupFFutureContractBindingV1,
...
export interface GroupFCreationResultRoleV1
{
  readonly roleKind: 'fixed'
  readonly name: 'media' | 'target'
}

export type GroupFCreationOperationV1 =
  | SemanticEditOperationMediaAddCostumeV1
  | SemanticEditOperationMediaAddSoundV1
  | SemanticEditOperationTargetAddSpriteV1

interface GroupFCreationContentInputV1 extends SemanticCreationNormalizationContextV1
{
  readonly operation: GroupFCreationOperationV1
  readonly descriptor: GroupFCreationBindingDescriptorV1
  readonly resultRole: GroupFCreationResultRoleV1
}
...
function expectedResultShape(operation: GroupFCreationOperationV1): {
...
function assertOperationRole(input: GroupFCreationContentInputV1): void
{
...
function ownerTargetRef(
  input: GroupFCreationContentInputV1,
...
function assertCreationScope(input: GroupFCreationContentInputV1): void
{
...
function resultInitialContent(input: GroupFCreationContentInputV1): unknown
{
  const { operation } = input
...
function groupFCreationContentProjectionForResultV1(
  input: GroupFCreationContentInputV1
): unknown
{
...
export function groupFCreationContentFingerprintForResultV1(
  input: GroupFCreationContentInputV1
): string
{
  return semanticHashV1(
    'semantic-fingerprint',
    groupFCreationContentProjectionForResultV1(input)
  )
}
