// packages/ir/src/edit/operations/procedure-operations.ts
...
} from '../graph/graph-primitives.js'
import {
...
import type { SemanticReferenceIndex } from '../semantic-index/reference-index-types.js'
...
): ProcedureRecordV1
{
  ...
  for (const [definitionBlockId, entry] of Object.entries(target.blocks))
  {
    ...
    matches.push({
      ...
      prototypeBlockId,
      proccode,
      warp: decodeProcedureWarpV1(mutation.warp).warp ?? false,
...
type ProspectiveProcedureRecordRoleV1 = 'definition' | 'prototype' | 'call'
...
): ProspectiveProcedureCollisionSetV1
{
  ...
  for (const [blockId, entry] of Object.entries(target.blocks))
  {
    ...
    records.push({
      ...
      warp: decodeProcedureWarpV1(mutationSource.warp).warp,
      ownership,
    })
  }
...
type ProcedureOperationKindV1 = (typeof PROCEDURE_OPERATION_KINDS_V1)[number]
