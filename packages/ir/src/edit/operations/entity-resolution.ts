// packages/ir/src/edit/operations/entity-resolution.ts
...
import {
  ...
  commentTextSha256V1,
} from '../graph/comment-operations.js'
...
import { decodeProcedureWarpV1 } from '../semantic-index/procedure-mutation.js'
import {
...
} from '../semantic-index/reference-index.js'
import {
  blockKey,
  declarationKey,
  scriptKey,
...
interface EntityEvidenceBuildContextV1
{
...
export interface EntityResolverAdaptersV1
{
...
{
  throw new EntityResolutionError(code, message)
}
...
function entityEvidenceBuildContextV1(
  project: ProjectIR,
  suppliedIndex?: SemanticReferenceIndex
): EntityEvidenceBuildContextV1
{
...
function targetLocationV1(
  target: Target,
  targetIndex: number,
  context?: EntityEvidenceBuildContextV1
): TargetLocationV1
{
...
function entityOrderedMatchSetSha256V1(
...
): T
{
  ...
  if (
    ...
    entityOrderedMatchSetSha256V1(entityKind, matches) !==
...
): readonly DeclarationEntityEvidenceV1[]
{
  return declarationEntityEvidenceSetFromContextV1(
    entityEvidenceBuildContextV1(project, suppliedIndex)
  )
}
...
): ProcedureLocationV1
{
  ...
  return {
    ...
    semanticFingerprint: semanticHashV1('semantic-fingerprint', {
      ...
      warp: decodeProcedureWarpV1(prototype.mutation?.warp).warp ?? false,
...
{
  const context = entityEvidenceBuildContextV1(project, suppliedIndex)
...
function resolveScopeV1(
  ...
  adapters: EntityResolverAdaptersV1,
  depth: number,
  context?: EntityEvidenceBuildContextV1
): { readonly targetIndex?: number; readonly script?: ScriptEntityEvidenceV1 }
{
  ...
  const script = resolveScriptRefInternalV1(
    ...
    adapters,
    depth + 1,
    context ?? entityEvidenceBuildContextV1(project)
  )
...
{
  const context = entityEvidenceBuildContextV1(project)
...
): ScriptEntityEvidenceV1
{
  return resolveScriptRefInternalV1(
    ...
    adapters,
    0,
    entityEvidenceBuildContextV1(project)
  )
}
