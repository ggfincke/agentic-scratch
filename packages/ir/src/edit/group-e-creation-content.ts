// packages/ir/src/edit/group-e-creation-content.ts
...
} from './group-d-creation-content.js'
...
export type GroupECreationBindingDescriptorV1 = Without<
  GroupEFutureContractBindingV1,
  'bindingKey' | 'expectedCreationContentFingerprintSha256'
>

export interface GroupECreationResultRoleV1
{
...
export type GroupECreationOperationV1 =
  | SemanticEditOperationProcedureAddV1
  | SemanticEditOperationProcedureUpdateSignatureV1
  | SemanticEditOperationProcedureSetCallArgumentV1
  | SemanticEditOperationProcedureRemoveV1

interface GroupESelectedCreationSourceV1
{
  readonly proccode?: string
  readonly scriptTopBlockId?: string
}

interface GroupECreationContentInputV1 extends SemanticCreationNormalizationContextV1
{
  readonly operation: GroupECreationOperationV1
  readonly descriptor: GroupECreationBindingDescriptorV1
  readonly resultRole: GroupECreationResultRoleV1
  readonly selectedSource: GroupESelectedCreationSourceV1
}
...
function expectedResultRoleShape(name: GroupECreationResultRoleV1['name']): {
...
function resultAlias(input: GroupECreationContentInputV1): string
{
...
function assertOperationRole(input: GroupECreationContentInputV1): void
{
  const { operation, descriptor, resultRole } = input
  const roles: Readonly<
    Record<GroupECreationOperationV1['kind'], readonly string[]>
  > = {
    'procedure.add': [
      'procedure',
      'definitionScript',
...
function ownerTargetRef(
  input: GroupECreationContentInputV1,
...
function assertCreationScope(input: GroupECreationContentInputV1): void
{
...
function signatureContent(input: GroupECreationContentInputV1): unknown
{
  const { operation } = input
  if (operation.kind !== 'procedure.add')
...
function resultInitialContent(input: GroupECreationContentInputV1): unknown
{
...
function groupECreationContentProjectionForResultV1(
  input: GroupECreationContentInputV1
): unknown
{
...
export function groupECreationContentFingerprintForResultV1(
  input: GroupECreationContentInputV1
): string
{
  return semanticHashV1(
    'semantic-fingerprint',
    groupECreationContentProjectionForResultV1(input)
  )
}
