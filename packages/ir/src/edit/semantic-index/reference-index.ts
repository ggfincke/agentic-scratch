// packages/ir/src/edit/semantic-index/reference-index.ts
...
interface MutableProcedure
{
  ...
  proccode: string
  warp: boolean | null
  warpEncoding: ProcedureWarpEncoding
  runtimeDefinition: BlockRef | null
  runtimePrototype: BlockRef | null
...
): SemanticReferenceIndex
{
  ...
  const procedure = (target: TargetRef, proccode: string): MutableProcedure =>
  {
    ...
    const created: MutableProcedure = {
      target,
      proccode,
      warp: null,
      warpEncoding: 'absent',
  ...
  )
  {
    ...
    for (const { blockId, entry } of blockEntriesByTarget[targetIndex]!)
    {
      ...
      if (entry.opcode === 'procedures_prototype')
      {
        ...
        procedure(targetReference, proccode).prototypeSourceOrder.push(
          ref(targetReference, blockId)
        )
        continue
      }
      if (entry.opcode !== 'procedures_definition') continue
      const input = ownRecordValue(entry.inputs, 'custom_block')
      const childId = input ? primaryInputSlot(input) : null
      const prototype =
        typeof childId === 'string'
      ...
      const indexed = procedure(targetReference, proccode)
      if (indexed.runtimeDefinition === null)
      {
        const decoded = decodeProcedureWarpV1(prototype.mutation?.warp)
        indexed.warp = decoded.warp
        indexed.warpEncoding = decoded.encoding
        indexed.runtimeDefinition = ref(targetReference, blockId)
        indexed.runtimePrototype = ref(targetReference, childId)
      }
    }
  }
