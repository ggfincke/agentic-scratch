// packages/ir/src/edit/semantic-index/procedure-mutation.ts
// bounded strict parsing for edit-owned custom-procedure mutation fields
...
import { ownRecordEntries, ownRecordKeys } from '../support/own-record.js'
...
export type ProcedureWarpEncoding =
  'boolean' | 'string' | 'null' | 'absent' | 'malformed'

export interface DecodedProcedureWarpV1
{
  // null when the flag is absent or malformed (the VM would throw on malformed)
  readonly warp: boolean | null
  readonly encoding: ProcedureWarpEncoding
}

// canonical warp decode mirroring the pinned VM (sequencer.js stepToProcedure):
// booleans honored as-is, strings via JSON.parse -> truthiness ("null" falsy),
// literal null is non-warp; admission's narrower set is validOptionalBooleanEncoding
export function decodeProcedureWarpV1(
  value: boolean | string | null | undefined
): DecodedProcedureWarpV1
{
  if (value === undefined) return { warp: null, encoding: 'absent' }
  if (typeof value === 'boolean') return { warp: value, encoding: 'boolean' }
  if (value === null) return { warp: false, encoding: 'null' }
  // non-string non-boolean never reaches the VM's JSON.parse arm -> malformed
  if (typeof value !== 'string') return { warp: null, encoding: 'malformed' }
  try
  {
    const parsed: unknown = JSON.parse(value)
    return { warp: Boolean(parsed), encoding: 'string' }
  }
  catch
  {
    return { warp: null, encoding: 'malformed' }
  }
}
